...
function parseOptionalDatetime(datetime) {
...
function getStartIso(session, events = parseHisEvents(session.notes)) {
    const startEvent = events.find((event) => event.type === 'START');
    return (startEvent?.at ??
        (session.created_at ? new Date(session.created_at).toISOString() : null) ??
        dateOnlyToIso(session.date_start) ??
        nowIso());
}
function getLastStatus(session, events = parseHisEvents(session.notes)) {
    const statusEvents = events.filter((event) => ['START', 'RESUME', 'PAUSE', 'STOP'].includes(event.type));
    const last = statusEvents.at(-1);
    if (session.date_end || last?.type === 'STOP')
        return 'stopped';
    if (last?.type === 'PAUSE')
        return 'paused';
    return 'active';
}
function getLastMood(events) {
    return events.filter((event) => event.type === 'MOOD').at(-1)?.text ?? 'none';
}
function activeDurationMs(session) {
...
async function appendToSession(session, type, body, at = nowIso()) {
...
async function stopSession(session, at = nowIso()) {
...
function countHisEntries(session) {
    return parseHisEvents(session.notes).length;
}
function eventSummary(session, limit = 5) {
...
server.registerTool('his_seshstart', {
    description: 'Start a Mental Health Incident System tracker session in drug_tracker_session. If current session is paused, resume it. Maps to /his-seshstart [optional datetime].',
    inputSchema: {
        datetime: z
            .string()
            .optional()
...
server.registerTool('his_seshstop', {
    description: 'Pause or stop the current tracker session. First run pauses. Running again while paused stops and sets date_end. Maps to /his-seshstop.',
    inputSchema: {},
}, async () => {
...
server.registerTool('his_export', {
...
server.registerTool('his_schema', {
