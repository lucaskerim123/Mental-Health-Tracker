...
export async function DELETE(req: NextRequest) {
  ...
  if ((target.role === 'admin' || target.role === 'owner') && !(await isAdminOwner(user.id))) {
    return NextResponse.json({ error: 'Only the owner can delete admin or owner accounts' }, { status: 403 })
  }
